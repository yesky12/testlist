#!/bin/sh
mysql_command="mysql -C -hsss -pcx2iYageUbZ3PSXR -P3316 -uotsmisc --default-character-set=utf8 -A -N -s -e"
...
do
{
	...
	do
		a=`nmap -P0 -p1723 $ip | grep open`
		if [ ${#a} != 0 ]; then
			browser=`cat /home/q/rsyncd/otsmisc/u*/* | grep "$ip" | grep "Firefox"`
			if [ ${#browser} != 0 ]; then
